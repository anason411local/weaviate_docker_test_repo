...
def create_collection():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            delete_response = requests.delete(f"{WEAVIATE_URL}/v1/schema/PDFDocuments", headers=headers)
            if delete_response.status_code == 200:
                print("✅ Successfully deleted existing PDFDocuments collection")
            else:
                print(f"⚠️ Failed to delete existing collection: {delete_response.status_code}")
                print(delete_response.text)
                # If we can't delete, don't try to create
                return False
    except Exception as e:
